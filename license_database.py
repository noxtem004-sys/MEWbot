...
# Try to import from core, fallback to simple path
try:
    from core.config_manager import app_data_dir
    LICENSE_DB_PATH: str = str(app_data_dir() / "licenses.db")
except ImportError:
    # For Railway/server deployment, use environment variable or default path
    LICENSE_DB_PATH: str = os.environ.get(
        'LICENSE_DB_PATH',
        '/app/data/licenses.db' if os.path.exists('/app') else './licenses.db'
    )
    # Ensure directory exists
    os.makedirs(os.path.dirname(LICENSE_DB_PATH) if os.path.dirname(LICENSE_DB_PATH) else '.', exist_ok=True)
